function h = semilogx_with_bar(x, y, sy, varargin)
% SEMILOGX_WITH_BAR Semilog plot (log x-axis) with symmetric error bars (Octave compatible)
%
% Usage:
%   h = semilogx_with_bar(x, y, sy)
%   h = semilogx_with_bar(x, y, sy, 'PropertyName', PropertyValue, ...)
%
% Inputs:
...
% Example:
%   x = logspace(0, 3, 20);
%   y = log10(x) + randn(size(x)) * 0.1;
%   sy = 0.2 * ones(size(x));
%   h = semilogx_with_bar(x, y, sy, 'color', 'r', 'marker', 's');
%
% See also: plot_with_bar, semilogy_with_bar, loglog_with_bar
